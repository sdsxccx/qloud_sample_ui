<template>
  <div
    class="g-h_container"
    :style="{backgroundColor:!meunAsideStyle ? menuStyle.menuBg : menuStyle.menuHeadBg}"
    :class="{'has-logo':showLogo,'has-aside':meunAsideStyle}"
  >
    <logo v-if="showLogo" :meun-aside-style="meunAsideStyle" :meun-head-style="meunHeadStyle"/>
    <el-menu
      :default-active="activeMenu"
      :background-color="!meunAsideStyle ? menuStyle.menuBg : menuStyle.menuHeadBg"
      :text-color="!meunAsideStyle ? menuStyle.menuText : menuStyle.menuHeadText"
    ...
    <div v-if="meunHeadStyle" class="right-menu">
      <a href="javascript:;" class="screen" :class="isFullscreen ? 'exit-screen' : 'full-screen'"
         :title="isFullscreen ? $t('public.exitFullScreen') : $t('public.screen')" @click="handleScreen">
      ...
      <div class="m-dropdown">
        <div class="m-photo">
          ...
          <image-upload :readonly="true" :widthHeight="22" :fileId="headPortrait"></image-upload>
        </div>
        <el-dropdown @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="(item, index) in droplist" :key="index" :command="item.command">{{
                item.name
              }}
            </el-dropdown-item>
          </el-dropdown-menu>
    ...
    >
      <template slot="content">
        <el-form
          class="passwordForm"
          ref="passwordForm"
          :model="passwordForm.formData" label-width="90px"
          :rules="passwordForm.formRules"
        >
          <el-form-item :label="$t('public.user.username')">
            <p>{{ userInfo.userName }}</p>
          </el-form-item>
          <el-form-item :label="$t('public.user.oldPassword')" prop="oldLoginPwd">
            <el-input
              type="password"
          ...
          <el-form-item :label="$t('public.user.newPassword')" prop="newLoginPwd">
            <el-input
              type="password"
          ...
          <el-form-item :label="$t('public.user.againLoginPwd')" prop="againLoginPwd">
            <el-input
...
import {queryTellerById, queryPlanByUserLoginId, modifyPwd} from '@/api/login/user'
  ...
  methods: {
    ...
    queryPlanByUserLoginId() {
      ...
      queryPlanByUserLoginId(sendData).then((response) => {
        if (isInclude(response, 'pwdType') && isType.isString(response.pwdType)) {
        ...
        this.$refs['passwordForm'].validate((valid) => {
          if (valid) {
            const sendData = {
              ...
              againLoginPwd: encodepwdSM3(this.passwordForm.formData.againLoginPwd),
            };
