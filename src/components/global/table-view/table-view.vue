/* * @Author: liguanghui * @Date: 2020-07-23 16:11:27 * @Last Modified by:
liguanghui * @Last Modified time: 2020-07-24 09:16:26 */
<template>
  <div class="m-table" :style="{ height: height }">
    ...
    >
      <!-- 单选列 -->
      <el-table-column v-if="isRidio" fixed width="50">
      ...
      />
      <!-- 表头列 -->
      <template v-for="(tHeaderItem, index) in tableHeaderData">
        <el-table-column
          header-align="left"
      ...
      >
        <template slot-scope="scope">
          <div class="operate">
            <a
              v-for="(item, index) in operateList(scope.row).slice(0, 2)"
              :key="item.id"
              class="p12 c6"
              :title="item.label"
              :style="{ color: theme }"
            ...
            <el-dropdown v-if="operateList(scope.row).length > 2">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  v-for="(item, index) in operateList(scope.row).slice(2)"
                  :key="item.id"
                  @click.native="
                    () => {
    ...
    tableData: {
      handler(newArr, oldArr) {
        ...
        if (this.height === "auto") {
          ...
          $self.isPage
            ? ($self.innerHeight = `calc(${this.height} - 54px)`)
